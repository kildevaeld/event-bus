...
namespace
{
    struct RemoveIfReciver : public std::unary_function<const Emitter::EventBus::SubscribersType&, bool>
    {
        RemoveIfReciver(QObject *receiver) : _receiver(receiver) {}

        bool operator()(const Emitter::EventBus::SubscribersType& item) const {
    ...
    struct FindIfReciver : public std::unary_function<const Emitter::EventBus::SubscribersType&, bool>
    {
        FindIfReciver(QThread *thread) : _thread(thread) {}
        bool operator()(const Emitter::EventBus::DispatchersType &item) const {
